...
Page({
    ...
    onShow: function() {
        ...
        if (token) {
            app.register
                .userInfo({}, { Authorization: `Bearer ${token}` })
                .then(res => {
                    if (res.is_signed == 1) {
                        wx.showToast({
                            title: '您已注册过该应用',
    ...
    submitForm: function() {
        ...
        if (
            this.data.name != null &&
            this.data.studentNum != null &&
            this.data.teacherNum != null &&
            ...
            app.register
                .signUp(data, { Authorization: `Bearer ${token}` })
                .then(
                    res => {
                        console.log('-----------------------------------');
                        console.log(`info is ${JSON.stringify(res)}`);
                        console.log('-----------------------------------');
                        // alert(`userInfo is ${JSON.stringify(res)}`)
                        wx.setStorageSync('userInfo', res);
                        wx.showToast({
                            title: '提交成功',
                            icon: 'none',
